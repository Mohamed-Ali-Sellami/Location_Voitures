...
carRouter.put('/reservation/:id', async (req, res) => {
  try {
    const result = await Car.findByIdAndUpdate(req.params.id, { $push:{ reservation :req.body }}, { new: true }); // Added { new: true } to return the updated document
    if (!result) {
      return res.status(404).send({ msg: "Voiture non trouvée" });
    }
    res.send({ car: result, msg: "Voiture mise à jour" });
