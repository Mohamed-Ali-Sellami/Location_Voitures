        ...
        <h3>Model Réservé:</h3>
          ...
          <div key={data._id}>
            ...
            <input
              type="date"
              value={startDate}
              onChange={(e) => setaddreservation({...addreservation,startDate:e.target.value})}
            />
            ...
            <input
              type="date"
              value={endDate}
              onChange={(e) => (setEndDate(setaddreservation({...addreservation,endDate:e.target.value})}  // onChange={(e) => setEndDate(e.target.value)}
            />

            <h3>Total Price:</h3>
            <input type="text" value={totalPrice ? `${totalPrice} TND` : '0 TND'} readOnly  onChange={(e) => setaddreservation({...addreservation,totalPrice:e.target.value})}  />
          </div>
       

        <button type="submit" className='confirmerreservation'  onClick={() => 
              dispatch(addreservation({id:data._id,reservation:addreservation}))}  >
          Confirmer
        </button>
        </>
         ))}
