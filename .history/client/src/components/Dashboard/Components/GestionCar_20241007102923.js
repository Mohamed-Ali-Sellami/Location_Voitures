...
const GestionCar = () => {
  ...
  const [upcar, setupclient] = useState({
    name:"" 
        })
...
price_per_day } TND</td>
                ...
                <td data-label="settings">
                    ...
                    {showsecond? 
                    (<>  
                    <input type="text" placeholder={data.name} onChange={(e)=>setupcar({name:e.target.value})}/> 
                    <button className="btn-up-ok" onClick={()=>dispatch(updatecar({id:data._id,updatecar:updatecar}))}>ok</button>
                     </>):null}
