...
// faza t7ot souris fil id yatla3 name(tooltip)
const [tooltip, setTooltip] = useState({ visible: false, info: ''});
console.log(tooltip)

const handleMouseEnter = (info) => {
  users.filter((el) => (el._id === info)? setTooltip({ visible: true, el}):null )
 ;
};
...
{tooltip.visible && (
        <div className='name-of-id'>
          {tooltip.el.name} {tooltip.el.lastname}
        </div>
  ...
  {carnews?.length > 0 ? (
    carnews.map((el, index) => (
      ...
      onMouseLeave={handleMouseLeave}>{el?.userId}</td>
        ...
        <td data-label="totalprice" className='tdadmin'>{el?.totalPrice} TND</td>
      </tr>
    ))
  ) : (
    <tr>
