...
const [tooltip, setTooltip] = useState({ visible: false, info: '', x: 0, y: 0 });

const handleMouseEnter = (e, info) => {
  const x = e.pageX;
  const y = e.pageY;
  setTooltip({ visible: true, info, x, y });
};

const handleMouseLeave = () => {
  setTooltip({ visible: false, info: '', x: 0, y: 0 });
};
...
<div className='bodygestionuser'>
  ...
  {carnews?.length > 0 ? (
    carnews.map((el, index) => (
      <tr key={index}>
        <td data-label="Utilisateur" onMouseEnter={(e) => handleMouseEnter(e, 'Informations supplémentaires ici')}
    ...
    {tooltip.visible && (
        <div
          className="tooltip"
          style={{ left: '300px', top: `${tooltip.y}px` }}
        >
          {tooltip.info}
        </div>
      )}
