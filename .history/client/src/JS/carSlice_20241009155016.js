...
//////////////////////////////reservation///////////////////////////////////////////////// (update lil car )
  export const newreservation=createAsyncThunk('/addreservation',async({id,reservation})=> {

    try {
    
     let resultat= axios.put(`http://localhost:5600/car/reservation/${id}`,reservation)  
     return await resultat;
...
.addCase (newreservation.pending , (state) => {
  ...
  .addCase (newreservation.fulfilled, (state) => {
  state.status ="success"
  
  })
  
  .addCase(newreservation.rejected , (state) => {
  state.status="fail";
  })
  }
})
